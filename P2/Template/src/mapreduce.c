...
int main(int argc, char *argv[]) {
	...
	if (nMappers >= ENDTYPE || nMappers >= ACKTYPE){
		printf("ERROR: Too many mappers, interfere with message queue \n");
		exit(0);
	}

	if (nReducers >= ENDTYPE || nReducers >= ACKTYPE){
		printf("ERROR: Too many reducers, interfere with message queue \n");
		exit(0);
	}
